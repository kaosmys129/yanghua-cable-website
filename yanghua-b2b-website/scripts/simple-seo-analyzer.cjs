...
class SimpleSEOAnalyzer {
  ...
  async run() {
    ...
    try {
      ...
      for (const page of config.pages) {
        ...
        try {
          const result = await this.analyzePage(fullUrl, page);
  ...
  async analyzePage(url, path) {
    try {
      // 使用lighthouse命令行工具
      const tempFile = `/tmp/lighthouse-${Date.now()}.json`;
      const command = `npx lighthouse "${url}" --only-categories=seo,performance,accessibility,best-practices --output=json --output-path="${tempFile}" --chrome-flags="--headless --no-sandbox" --quiet`;
      
      execSync(command, { stdio: 'pipe' });
      
      // 读取结果文件
      const resultData = JSON.parse(fs.readFileSync(tempFile, 'utf8'));
      
      // 清理临时文件
      fs.unlinkSync(tempFile);
      
      return this.processLighthouseResult(url, path, resultData);
    } catch (error) {
      throw new Error(`Lighthouse分析失败: ${error.message}`);
    }
  }
  ...
  async generateReports(processedResults) {
    ...
    const jsonFilename = `simple-seo-analysis-${this.timestamp}.json`;
    const htmlFilename = `simple-seo-analysis-${this.timestamp}.html`;
    const csvFilename = `simple-seo-analysis-${this.timestamp}.csv`;
