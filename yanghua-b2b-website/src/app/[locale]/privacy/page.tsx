...
export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  ...
  const pageContent = contentRepository.getPageContent<LegalPageContent>('privacy', locale as 'en' | 'es');
  ...
  return {
    title: pageContent?.content.title || 'Privacy Policy | Yanghua Cable',
    description:
      pageContent?.content.description ||
      'Privacy Policy for Yanghua Cable website and flexible busbar systems services.',
    alternates: {
      canonical,
      languages,
    },
...
export default async function PrivacyPage({ params }: { params: { locale: string } }) {
  ...
  const pageContent = contentRepository.getPageContent<LegalPageContent>('privacy', locale)?.content;
  const sections = Array.isArray(pageContent?.sections) ? pageContent.sections : [];
  const fallbackTitle = locale === 'es' ? 'Politica de privacidad' : 'Privacy Policy';
  ...
  const fallbackContactDescription =
    locale === 'es'
      ? 'Para cualquier consulta sobre privacidad, ponte en contacto con el equipo de Yanghua.'
      : 'For any privacy-related questions, please contact the Yanghua team.';
    ...
    <div className="min-h-screen bg-white">
      <div className="bg-gradient-to-r from-green-600 to-green-800 text-white py-16">
        <div className="container mx-auto px-4">
          ...
          <p className="text-green-100">{pageContent?.lastUpdated || fallbackUpdated}</p>
        </div>
      </div>
